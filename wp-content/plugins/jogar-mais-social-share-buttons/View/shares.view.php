...
class Shares_View extends Core
{
	...
	public static function ssb_share( $atts = array() )
	{
		...
		$atts = array_map( array( 'JM\Share_Buttons\Utils', 'esc_class' ), $atts );
		$args = array(
	...
	private static function _start_buttons_html( \stdClass $args )
	{
		...
		$content  .= ( Utils::option( 'position_fixed' ) ) ? '<div data-element="buttons">' : '';
	...
	{
		$op_inside  = Utils::option( 'remove_inside', false, 'intval' );;
	...
	{
		$remove_count   = Utils::option( 'remove_count', false, 'intval' );
		$counter        = ( $set_count ? $set_count : $remove_count );
		$position_fixed = Utils::option( 'position_fixed' );
	...
	{
		if ( Utils::option( 'remove_count', false, 'intval' ) )
			return;
	...
	{
		if ( Utils::option( 'position_fixed' ) ) :
			$prefix = Setting::PREFIX;
			$close  = '</div>';
	...
	{
		$position_left = Utils::option( 'position_fixed', false );
	...
	public static function buttons_share( $atts = array() )
	{
		$atts = array_map( array( 'JM\Share_Buttons\Utils', 'esc_class' ), $atts );
		$args = array(
			'class_first'  => Utils::isset_set( $atts, 'class_first' ),
			'class_second' => Utils::isset_set( $atts, 'class_second' ),
			'class_link'   => Utils::isset_set( $atts, 'class_link' ),
			'class_icon'   => Utils::isset_set( $atts, 'class_icon' ),
			'layout'       => Utils::isset_set( $atts, 'layout' ),
			'elements'     => array(
				'remove_inside'  => Utils::isset_set( $atts, 'remove_inside' ),
				'remove_counter' => Utils::isset_set( $atts, 'remove_counter' ),
			),
		);
