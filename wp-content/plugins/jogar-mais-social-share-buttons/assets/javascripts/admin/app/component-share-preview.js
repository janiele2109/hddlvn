JOGARMAIS( 'SSB.Components.SharePreview', function(SharePreview, $) {

	SharePreview.prototype.start = function(container) {
		this.preview       = container;
		this.spinner       = $( '.ajax-spinner' );
		this.prefix        = SSB.vars.prefix;
		this.container     = container.closest( '.ssb-share-wrap' );
		...
		this.elements      = $( '.ssb-select-item input' );
	...
	SharePreview.prototype._onClickLayout = function(event) {
		if ( 'fixed-left' !== event.currentTarget.value ) {
			$( '.layout-fixed, .ssb-fixed' ).prop( 'checked', false );
	...
	SharePreview.prototype._done = function(response) {
		...
		SSB.Preview.create( this.preview );
	};
	...
	SharePreview.prototype.render = function(response) {
		return JOGARMAIS.Templates[this.templateName()]
		                .call( null, response );
	};
